...
const dictionaryPath = path.join(rootDir, 'zd-extension/js/vnedict.json');
...
${runtimeSources}
  ...
  // Generated by scripts/build-popupdict-userscript.js from zd-extension/js/vnedict.json.
  ...
  function renderDefinition(entry) {
    ...
    return [
      ...
      '<ul>',
      definitions.map(renderDefinitionItem).join(''),
      '</ul>',
      '</div>'
    ].join('');
  }
  ...
  function main() {
    ...
    window.addEventListener('mouseout', clearActiveResult);
...
const entries = JSON.parse(fs.readFileSync(dictionaryPath, 'utf8'));
