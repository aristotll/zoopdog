...
2. Ignore blank lines, Markdown headings, horizontal rules, and fence markers for file input. Split ordinary content on newlines, commas, semicolons, or `|`; recognize a three-field `Vietnamese / ChuNom / explanation` item before separator expansion. When a non-slash line contains CJK annotations, switch to filtered-input mode: treat the entire line as one phrase, discard CJK and other non-Latin punctuation, preserve Vietnamese/Latin letters and collapsed word spacing, and discard a line that has no Vietnamese/Latin content. Embedded CJK is never carried into a candidate.
...
6. Consult sources in a fixed precedence: user entries for skip detection, then `vnedict2.json`, then optional `mdx_nom.json`. Extract CJK/Nom values and English definitions with stable source order and stable de-duplication. Slash triples remain explicit user evidence. Mixed inputs are tagged `input-filtered`; their candidates come only from the cleaned Vietnamese lookup and always remain `needs-review` for the AI review layer.
...
The manifest has a schema version and repository-state snapshot containing SHA-256 hashes for every source file that influenced the plan, including the selected input file when applicable. Entries are classified as `proposed`, `needs-review`, or `skipped`. The reviewer may change `vi`, `nom`, and `explain`, then set each valid, fully reviewed actionable entry's decision to `apply` by default; explicit rejection or unresolved linguistic data remains `reject` and is never written.
...
The JSONC updater uses a small lexical scanner that understands strings, escapes, line comments, and block comments. It identifies top-level entry objects and property value spans. Before mutation, approved input entries are grouped by normalized Vietnamese key with stable Nom/explanation de-duplication. Existing entries are updated by replacing only the `vi`, `nom`, and `explain` JSON values, preserving surrounding whitespace and comments; new entries are inserted before the top-level closing bracket using the file's established indentation and newline style. Reapplying the same approved entry to the same JSONC bytes is byte-idempotent. The updated document is parsed through the same comment-stripping rules before replacement of the original file.
...
For `input-filtered` candidates, the command explicitly checks exact phrase meaning and local dictionary evidence, supplies or corrects the Nom form and a concise explanation when deterministic sources are incomplete, and presents the reviewed entry for user approval. Filtered input characters are not evidence and must not be restored. After approval, it defaults every complete reviewed entry to `decision: apply`, keeps explicit rejections or unresolved entries at `decision: reject`, and invokes Node.js for the only dictionary write. The AI review edits only the temporary manifest; deterministic apply remains the sole mutation path.
