...
- [x] 2.5 Make runtime JSON and metadata publication failure-atomic as a pair, with an injected second-publication failure test proving the previous matching pair remains intact
...
- [x] 3.5 Preserve force semantics when `ensureReady({force:true})` arrives during an ordinary in-flight readiness call, with a regression test proving a replacement occurs exactly once
