...
Zoopdog is a static website for a Vietnamese-English popup dictionary and Vietnamese
pronunciation tools, along with two standalone userscripts generated from the same data. There
is no browser extension: `zd-extension/` is a legacy directory name that now just holds the
shared dictionary source data (`db_src/`) and the shared browser runtime (`js/`) the website
and userscripts are built from.
...
| `zd-extension/` | Legacy name; shared dictionary data (`db_src/`) and shared browser runtime/styles used by the website and userscripts |
| `zd-extension/js/zd-words.js` | Shared Vietnamese word primitives — one definition, used by the website and the popup userscript |
...
- Preserve Vietnamese text and diacritics exactly when touching dictionary, pronunciation, or
  UI copy.
- Do not delete binary/image/font assets unless the task explicitly asks for asset cleanup.
...
- **Never run `git checkout --`, `git stash`, or any other discard on
  ...
  (interactive or background) editing the hand-maintained Chu Nom shard store live. An
  unexpected diff there during your own work is very likely someone else's in-progress edit,
  not pollution from your own process — discarding it destroys their work. If a test fails
  because of dictionary-derived content, rebuild the generated artifact
