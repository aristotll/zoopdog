...
test('a failed metadata publication leaves the previous matching dictionary/metadata pair intact', () => {
  ...
  const previousBytes = serializeRuntimeDictionary(groupEntries([{vn: 'chó', en: [{def: 'dog', pos: ''}]}]));
  ...
  const nextBytes = serializeRuntimeDictionary(groupEntries([{vn: 'mèo', en: [{def: 'cat', pos: ''}]}]));
...
test('a failed metadata publication with no prior dictionary leaves neither file behind', () => {
  ...
  const bytes = serializeRuntimeDictionary(groupEntries([{vn: 'mèo', en: [{def: 'cat', pos: ''}]}]));
