...
Help resolve linguistic ambiguity only. Record the reviewed values in the manifest. After the user approves the review, set `decision` to `apply` by default for every complete entry with valid `vi`, `nom`, and reviewed `explain`; set `decision` to `reject` for explicit rejections and anything still unresolved.

Node.js is the only writer: do not edit `zd-extension/db_src/user_nom_entries.jsonc`, clean the input file, or rebuild generated files directly. Its approved-entry upsert is duplicate-free and idempotent by normalized Vietnamese key. Do not invoke apply before the user explicitly approves the reviewed proposal.
